...
const Vacancies = () => {
  ...
  const handleChange = (e, index) => {
    const { name, value } = e.target;
    if (name === "requirments") {
      const newRequirments = [...formData.requirments];
      newRequirments[index] = value;
      setFormData((prevState) => ({
        ...prevState,
        requirments: newRequirments,
      }));
    } else {
      setFormData((prevState) => ({
        ...prevState,
  ...
  const validateForm = (data) => {
    ...
    if (!data.category.trim()) errors.category = "Category is required";
    if (!data.description.trim()) errors.description = "Description is required";
    if (data.requirments.some((req) => !req.trim())) errors.requirments = "All requirements must be filled";
    return errors;
  };
          ...
          {filteredVacancies.map((vacancy) => (
            <tr key={vacancy._id}>
              ...
              <td style={{ display: 'flex', justifyContent: 'center', alignItems: 'center' }}>
                <Button variant="info" className="mr-2" onClick={() => handleEdit(vacancy)} style={{ marginRight: '10px' }}>
                  <IoMdCreate />
                </Button>
                <Button variant="danger" onClick={() => handleOpenConfirmationModal(vacancy._id)} style={{ marginLeft: '10px' }}>
                  <IoMdTrash />
                </Button>
              </td>
            </tr>
      ...
      {editVacancy && (
        <div style={{ margin: '20px 0', padding: '20px', border: '1px solid #ddd', borderRadius: '10px', backgroundColor: '#f9f9f9' }}>
          ...
          <form onSubmit={handleSubmit}>
            ...
            <div className="form-group" style={{ marginBottom: '20px' }}>
              <label>Requirements:</label>
              {formData.requirments.map((req, index) => (
                <div key={index} style={{ display: 'flex', alignItems: 'center', marginBottom: '10px' }}>
                  ...
                  />
                  {formData.requirments.length > 1 && (
                    <Button
                      variant="danger"
                      onClick={() => {
                        setFormData((prevState) => ({
                          ...prevState,
                          requirments: prevState.requirments.filter((_, i) => i !== index),
                        }));
                      }}
              ...
              <Button
                ...
                onClick={() => {
                  setFormData((prevState) => ({
                    ...prevState,
                    requirments: [...prevState.requirments, ""],
                  }));
                }}
              >
                Add Requirement
              </Button>
              {errors.requirments && <span className="error" style={{ color: 'red' }}>{errors.requiremnts}</span>}
            </div>
            <Button variant="primary" type="submit" style={{ width: '100%', padding: '10px', borderRadius: '5px' }}>
              Update Vacancy
            </Button>
